...
fltk3::Slider *zSlider1, *zSlider2;
...
class MyGLView : public fltk3::GLWindow
{
public:
...
//      glBlendFunc(GL_ONE, GL_ZERO);
    ...
    if (gShowSlice) {
      ...
      drawModelFlat(fltk3::RED);
      glMatrixMode (GL_PROJECTION);
      // change the z range to disable clipping
      glLoadIdentity();
      glOrtho(-66.1,66.1,-66.1,66.1,-66.1,66.1); // mm
      glMatrixMode(GL_MODELVIEW);
      gMeshSlice.drawFlat(fltk3::GREEN);
...
//        drawModelShrunk(fltk3::WHITE, sd);
//      }
...
static void xButtonCB(fltk3::Widget*, void*)
{
  gShowSlice = false;
  glView->redraw();
}

static void sliceCB(fltk3::Widget*, void*)
{
  gMeshSlice.clear();
  int i, n = (int)gMeshList.size();
  for (i=0; i<n; i++) {
...
static void z1ChangedCB(fltk3::Widget*, void*)
{
  sliceCB(0, 0);
  gShowSlice = true;
  glView->redraw();
...
static void z2ChangedCB(fltk3::Widget*, void*)
{
  gShowSlice = true;
  glView->redraw();
}

static void writeSliceCB(fltk3::Widget*, void*)
{
  double z;
#ifdef M_MONKEY
...
static void writePrnSliceCB(fltk3::Widget*, void*)
{
  double z;
#ifdef M_MONKEY
  double firstLayer  = -8.8;
